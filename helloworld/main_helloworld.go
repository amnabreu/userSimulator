...
func createResources(ctx context.Context, stackID gocql.UUID) context.Context {
	...
	request := core.ResourceCreateRequest{Name: "CloudRun_HelloWorld", Driver: core.DriverCloudrun.String(), Provider: "GCP", Immutable: true, StackID: stackID, Config: `{"properties":{"generation":"second-generation","cpu":"2000m","memory":"1024Mi"},"output":{"env":[{"url":"CloudRun_HelloWorld_URL"}]}}`}
...
func createWorkloads(ctx context.Context, stackID gocql.UUID) {
	...
	fmt.Printf("rsid: %v\n", rsid)
	request := core.WorkloadCreateRequest{Name: "hello_world", Kind: "worker", RepoID: repoID, StackID: stackID, RepoPath: "https://github.com/amnabreu/HelloWorld", ResourceID: rsid}
